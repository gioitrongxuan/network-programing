        ...
        if (currentUser == NULL)
        {
            ...
            else
            {
                ...
                case TOO_MANY_WRONG_PASSWORD:
                    ...
                    {
                        printf("%s\n",clientSession[i].username);
                        if(strcmp(clientSession[i].username,username) == 0)
                        {
                            printf("test logout");
                             send(clientSession[i].sockfd, "Tai khoan da bi block o noi nao do", strlen(buffer), 0);
                             kill(clientSession[i].pid,SIGUSR1);
                            // Bắt pid đó chạy hàm logout()
